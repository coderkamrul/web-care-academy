...
export default function ActionCursor({
  defaultIcon = <GoArrowUpRight className="w-10 h-10 text-black" />,
}) {
  const cursorRef = useRef(null)
  const [icon, setIcon] = useState(defaultIcon)
  const activeTargetRef = useRef(null)
  ...
  // Attach hover handlers — runs on pathname change + DOM mutations
  useEffect(() => {
    const cursor = cursorRef.current
    if (!cursor) return
    ...
    const attachHandlers = () => {
      const targets = document.querySelectorAll("[data-action-cursor]")
      targets.forEach((el) => {
        if (el._cursorBound) return // ✅ avoid duplicate binding
        const mouseEnter = () => show(el)
        const mouseLeave = () => hide(el)
        el.addEventListener("mouseenter", mouseEnter)
        el.addEventListener("mouseleave", mouseLeave)
    ...
    // Watch for DOM changes (e.g., new page content, reused components)
    const observer = new MutationObserver(() => attachHandlers())
    observer.observe(document.body, { childList: true, subtree: true })

    return () => observer.disconnect()
  }, [defaultIcon, pathname])
